...
const statusColor: Record<string, string> = {
  not_started: 'text-[var(--text-secondary)]', in_progress: 'text-blue-400',
    ...
    <div className="max-w-7xl mx-auto space-y-8 animate-fade-in">
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          ...
          <h1 className="text-3xl font-bold text-[var(--text-primary)] mt-0.5 tracking-tight">{user?.fullName}</h1>
          <div className="flex items-center gap-2 mt-1.5">
            <div className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse" />
            <p className="text-[var(--text-muted)] text-sm">{user?.workspaceName} · {user?.roleName}</p>
          </div>
        </div>
      ...
      {isManager && !loading && stats.total > 0 && (
        ...
        >
          ...
          <div className="relative flex items-center gap-8 flex-wrap">
            <div>
              <p className="text-[var(--text-secondary)] text-sm mb-1">Team Completion Rate</p>
              <div className="flex items-baseline gap-2">
                <span className="text-5xl font-bold text-[var(--text-primary)]">{completionRate}</span>
      ...
      {loading ? (
        ...
        >
          {[
            { label: 'Total Tasks', value: stats.total, icon: CheckSquare, color: 'text-indigo-400', bg: 'bg-indigo-500/10', sub: `${completionRate}% complete` },
            { label: 'In Progress', value: stats.inProgress, icon: Clock, color: 'text-blue-400', bg: 'bg-blue-500/10', sub: 'Active right now' },
            { label: 'Blocked', value: stats.blocked, icon: AlertTriangle, color: 'text-red-400', bg: 'bg-red-500/10', sub: stats.blocked > 0 ? 'Needs attention' : 'All clear' },
            { label: 'Completed', value: stats.done, icon: TrendingUp, color: 'text-emerald-400', bg: 'bg-emerald-500/10', sub: stats.overdue > 0 ? `${stats.overdue} overdue` : 'On track' },
          ].map(stat => (
      ...
      <div className="grid lg:grid-cols-5 gap-6">
        {/* Tasks — wider column */}
        <div className="lg:col-span-3 bg-[var(--bg-surface)]/80 border border-[var(--border)][0.06] rounded-2xl">
          <div className="flex items-center justify-between px-6 py-4 border-b border-[var(--border)][0.06]">
            <div className="flex items-center gap-2">
              <CheckSquare className="w-4 h-4 text-[var(--text-secondary)]" />
              <h2 className="font-semibold text-[var(--text-primary)]">Active Tasks</h2>
          ...
          <div className="p-4">
            {loading ? (
              ...
              <div className="text-center py-12">
                ...
                <p className="text-[var(--text-primary)] font-medium mb-1">All caught up!</p>
                <p className="text-[var(--text-muted)] text-sm">No active tasks. Create one to get started.</p>
                <Link href="/tasks/new">
                ...
                {activeTasks.map(task => (
                  <Link key={task.id} href={`/tasks/${task.id}`}>
                    <div className="group flex items-center gap-3 px-3 py-3 rounded-xl
                      ...
                      hover:border-[var(--border)][0.08]">
                      <div className={cn('w-2.5 h-2.5 rounded-full shrink-0 mt-0.5',
                        priorityDot[task.priority] ?? 'bg-slate-500')} />
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-medium text-slate-200 group-hover:text-[var(--text-primary)]
                          transition truncate">
                          {task.title}
                        </p>
                        {task.due_date && (
                      ...
                      <span className={cn('text-xs font-medium shrink-0',
                        statusColor[task.status] ?? 'text-[var(--text-secondary)]')}>
                        {statusLabel[task.status] ?? task.status}
                      </span>
                      <ArrowRight className="w-3.5 h-3.5 text-slate-700 group-hover:text-[var(--text-secondary)]
        ...
        <div className="lg:col-span-2 space-y-6">
          {/* Projects */}
          <div className="bg-[var(--bg-surface)]/80 border border-[var(--border)][0.06] rounded-2xl">
            <div className="flex items-center justify-between px-5 py-4 border-b border-[var(--border)][0.06]">
              <div className="flex items-center gap-2">
                <FolderKanban className="w-4 h-4 text-[var(--text-secondary)]" />
                <h2 className="font-semibold text-[var(--text-primary)]">Projects</h2>
              </div>
              <Link href="/projects">
                <Button variant="ghost" size="xs" iconRight={<ArrowRight className="w-3.5 h-3.5" />}>
                  All
                  ...
                  <Link key={project.id} href={`/projects/${project.id}`}>
                    <div className="group p-3 rounded-xl hover:bg-white/5 transition cursor-pointer">
                      <div className="flex items-center gap-2.5 mb-2">
                        ...
                        <span className="text-sm font-medium text-slate-300 group-hover:text-[var(--text-primary)]
                          transition truncate flex-1">
                          {project.name}
                        </span>
                        <span className="text-xs text-[var(--text-muted)] shrink-0">{project.progress}%</span>
                      </div>
              ...
              {[
                ...
                <Link key={action.href} href={action.href}>
                  <div className="flex flex-col items-center gap-2 p-3 rounded-xl
                    ...
                    <span className="text-xs text-[var(--text-secondary)] group-hover:text-[var(--text-primary)] transition font-medium">
                      {action.label}
                    </span>
                  </div>
                </Link>
