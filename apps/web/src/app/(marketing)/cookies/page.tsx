  ...
  return (
    <div className="min-h-screen bg-slate-950 pt-32 pb-24">
      <div className="max-w-3xl mx-auto px-6">
        ...
        <div className="space-y-8 text-slate-400 leading-relaxed mb-12">
          <div>
          ...
          <div>
            <h2 className="text-lg font-semibold text-white mb-3">Types of cookies we use</h2>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-white/[0.06] bg-slate-900/60">
                ...
                <th className="text-left px-4 py-3 text-slate-400 font-semibold">Expiry</th>
              </tr>
            </thead>
            <tbody>
              {COOKIES.map((c, i) => (
                <tr key={c.name}
                  className={i < COOKIES.length - 1 ? 'border-b border-white/[0.04]' : ''}>
                  <td className="px-4 py-3 text-white font-mono text-xs">{c.name}</td>
                  <td className="px-4 py-3 text-slate-400">{c.type}</td>
                  <td className="px-4 py-3 text-slate-500 max-w-xs">{c.purpose}</td>
