      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="text-center mb-16">
          ...
          >
            ...
            {/* Billing toggle */}
            <div className="inline-flex items-center gap-3 bg-[var(--bg-surface)] border border-[var(--border)]10
              rounded-2xl p-1.5">
              <button onClick={() => setAnnual(false)}
                className={cn('px-5 py-2.5 rounded-xl text-sm font-medium transition-all',
                  !annual ? 'bg-white text-slate-900' : 'text-[var(--text-secondary)] hover:text-[var(--text-primary)]')}>
                Monthly
              </button>
              <button onClick={() => setAnnual(true)}
                className={cn('px-5 py-2.5 rounded-xl text-sm font-medium transition-all flex items-center gap-2',
                  annual ? 'bg-indigo-600 text-[var(--text-primary)]' : 'text-[var(--text-secondary)] hover:text-[var(--text-primary)]')}>
                Annual
            ...
            <motion.div
              ...
              className={cn(
                ...
                plan.badge
                  ...
                  : 'bg-[var(--bg-surface)]/60 border-[var(--border)][0.06]'
              )}
            >
              {plan.badge && (
                <div className="absolute -top-3 left-1/2 -translate-x-1/2
                  bg-indigo-600 text-[var(--text-primary)] text-xs font-semibold px-4 py-1
              ...
              <Link href={plan.href}
                className={cn(
                  ...
                  plan.badge
                    ? 'bg-indigo-600 hover:bg-indigo-500 text-[var(--text-primary)] shadow-lg shadow-indigo-500/20'
                    : 'border border-[var(--border)]15 text-slate-300 hover:text-[var(--text-primary)] hover:border-[var(--border)]30'
                )}>
                {plan.cta}
              </Link>
            </motion.div>
